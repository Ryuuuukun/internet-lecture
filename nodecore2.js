...
class ObjectEvents {
    constructor(events) {
        ...
        events.add('mousedown', (event, canvas) => {
            ...
            let select = [...this.objects.values()].filter(({ object }) => object.collision(x, y)).sort((a, b) => b.index - a.index)[0];
            if (select !== undefined) {
        ...
        events.add('mousemove', (event, canvas) => {
            ...
            if (this.pressed) {
                let dx = event.clientX - this.lastMouseMove.getX();
                let dy = event.clientY - this.lastMouseMove.getY();
        ...
        events.add('mouseup', (event, canvas) => {
            ...
            if (!event.shiftKey) {
                ...
                let click = [...this.objects.values()].filter(({ object }) => object.collision(x, y)).sort((a, b) => b.index - a.index)[0];
        ...
        events.add('mousemove', (event, canvas) => {
            if (this.hovering) {
                ...
                this.objects.values().forEach(({ object, index }) => {
                    let inside = object.collision(x, y);
...
class NodeEngine {
    ...
    centerit() {
        let center = centermass(this.network.components);
        this.canvas.transform.offset.set(
            this.canvas.width  / 2 - center.getX(),
            this.canvas.height / 2 - center.getY()
        );
    }
